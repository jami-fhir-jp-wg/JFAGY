...
#### version: "2026.09.25" (2025.09.25修正リリース)
  - 青魚(J9FC11981000)、ナッツ類(J9FA98010000)を追加。
  - 以下の名称(JPTERM)を修正
    - 大福　→　大福豆
    - 果実　→　果実、種実
    - 果実（その他）　→　果実、種実（その他）
    ...
    - いか類（その他）　→　いか（その他）
    - いか類（詳細不明）　→　いか（詳細不明）
    - 麦芽,麦芽抽出物及び麦芽シロップ　→　麦芽、麦芽抽出物及び麦芽シロップ
    - 麦芽,麦芽抽出物及び麦芽シロップ（その他）　→　麦芽、麦芽抽出物及び麦芽シロップ（その他）
    - 麦芽,麦芽抽出物及び麦芽シロップ（詳細不明）　→　麦芽、麦芽抽出物及び麦芽シロップ（詳細不明）
  - 上記に合わせて英語表記、かなふりがな（JPKANA）を修正
  - かなふりがな（JPKANA）の誤記46件を修正

#### version: "2026.05.31" (2025.08.23修正リリース)
  - 以下の修正をした​JFAGY_J9FN_20260531_R02-utf8.zip、​JFAGY_J9FN_20260531_R02-sjis.zipをリリース
    - 2026-05-31リリース版で各カラムが二重引用符で囲まれていなかった点と、UTF-8ファイルでBOM有りになっていた点を修正。
    - zipファイルを展開した時にutf8フォルダまたはsjisフォルダが作成され、その中にファイルが展開されるように改善。

#### version: "2026.05.31" (2025.05.31修正リリース)
  - 以下について消費者庁による特定原材料等の表記に変更した。コード変更はなし。
    - 食用鳥卵 → 卵
    - 落花生 → 落花生（ピーナッツ）
    - いか類 → いか
    - マカデミアナッツ → マカダミアナッツ
